...
(function(){
    ...
    this.addNewline = function(before) {
        if (this.$readOnly)
            return;

        if (before) // Insert newline above cursor
            this.navigateUp();
    ...
    this.navigateLineStart = function(ignoreWhitespace) {
        if (ignoreWhitespace) {
            this.moveCursorTo(this.selection.selectionLead.row, 0);
        } else {
